...
/**
 * Plugin Name: X-Lazy (Lazy Load Images)
 * Plugin URI:  http://xtechnology.co
 * Description: Lazy Load WordPress images. Load images only after scrolling down and when viewport.
 * Author:      X Technology
 * Author URI:  https://www.xtechnology.co/
 * Text Domain: x-lazy-load-images
...
add_action( 'wp', array( 'X_Lazy_Load_Images', 'instance' ) );

final class X_Lazy_Load_Images {
	...
	public function __construct() {
		...
		//* XLLI Hooks
		add_filter( 'the_content', array( __CLASS__, 'xlli_images' ), 12 );
		add_filter( 'post_thumbnail_html', array( __CLASS__, 'xlli_images' ) );
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'xlli_effects' ) );
		add_action( 'wp_footer', array( __CLASS__, 'xlli_scripts' ) );
	}

	//* XLLI content images for Lazy Loaded
	public static function xlli_images( $content ) {
		...
		return preg_replace_callback( '/(?P<all> (?# ) <img(?P<before>[^>]*) (?# ) ( (?# ) class=["\'](?P<class1>.*?(?:wp-image-|wp-post-image)[^>"\']*)["\'] (?P<between1>[^>]*) src=["\'](?P<src1>[^>"\']*)["\'] | (?# ) src=["\'](?P<src2>[^>"\']*)["\'] (?P<between2>[^>]*) class=["\'](?P<class2>.*?(?:wp-image-|wp-post-image)[^>"\']*)["\'] ) (?P<after>[^>]*) (?# ) (?P<closing>\/?)> (?# ) )/x', array( 'X_Lazy_Load_Images', 'replace_images' ), $content );
	}
